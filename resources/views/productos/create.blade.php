...
<div class="max-w-2xl mx-auto">
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Crear Nuevo Producto</h3>
            <p class="mt-1 max-w-2xl text-sm text-gray-500">Agrega un nuevo producto a tu catálogo</p>
        </div>
        <div class="border-t border-gray-200">
            <form action="{{ route('productos.store') }}" method="POST" class="px-4 py-5 sm:p-6 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="descripcion" id="descripcion" rows="3"
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm">{{ old('descripcion') }}</textarea>
                    @error('descripcion')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="peso" id="peso" step="0.01" min="0" value="{{ old('peso') }}"
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm">
                        @error('peso')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
