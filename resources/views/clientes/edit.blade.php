...
<div class="max-w-2xl mx-auto">
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6">
            ...
            <p class="mt-1 max-w-2xl text-sm text-gray-500">Actualiza la información del cliente</p>
        </div>
        <div class="border-t border-gray-200">
            <form action="{{ route('clientes.update', $cliente) }}" method="POST" class="px-4 py-5 sm:p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="direccion" id="direccion" rows="3"
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('direccion', $cliente->direccion) }}</textarea>
                    @error('direccion')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="activo" class="block text-sm font-medium text-gray-700 mb-2">Estado del Cliente</label>
                    <div class="flex items-center space-x-4">
                        <label class="inline-flex items-center">
                            <input type="radio" name="activo" value="1" {{ old('activo', $cliente->activo) == '1' ? 'checked' : '' }}
                                   class="form-radio h-4 w-4 text-indigo-600 transition duration-150 ease-in-out">
                            <span class="ml-2 text-sm text-gray-700">Activo</span>
                        </label>
                        <label class="inline-flex items-center">
                            <input type="radio" name="activo" value="0" {{ old('activo', $cliente->activo) == '0' ? 'checked' : '' }}
                                   class="form-radio h-4 w-4 text-indigo-600 transition duration-150 ease-in-out">
